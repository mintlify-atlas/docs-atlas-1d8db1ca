...
The `WebsocketClient` extends Node.js `EventEmitter` and exposes two distinct error-related events:

| Event | When it fires |
|-------|---------------|
| `error` | A raw WebSocket transport error occurred (e.g. connection refused, TLS failure). |
| `exception` | The SDK encountered an internal error while processing a message, or your own event listener threw an exception. |
...
// SDK-level exceptions (includes exceptions thrown inside your own handlers)
ws.on('exception', (err) => {
  console.error('WebSocket SDK exception:', err);
});
...
<Note>
  The `exception` event also fires when an uncaught error is thrown inside any of your own `update`, `response`, or `authenticated` event handlers. This helps centralise error logging without silently dropping events.
</Note>
